...
TEST (TimedTransitionFire, BasicAssertions)
{
  ...
  auto inArch = InputArch::New(engine, inPlace);
  auto outArch = OutputArch::New(engine, outPlace);
  auto transition = TimedConstantTransition::New(engine, "Transition", {inArch}, {outArch}, 0);
...
TEST(TimedExpoonentialTransitionFire, BasicAssertions)
{
  ...
  auto inArch = InputArch::New(engine, inPlace);
  auto outArch = OutputArch::New(engine, outPlace);
  auto transition = TimedExponentialTransition::New(engine, "ExponentialTransition", {inArch}, {outArch}, 10);
  EXPECT_EQ(transition->allInputArchSsatisfied(), 1);
  transition->fire(-1);
  EXPECT_EQ(inPlace->tokens(), 0);
  EXPECT_EQ(outPlace->tokens(), 1); 
